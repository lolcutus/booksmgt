...
public class BooksDatabase {
    ...
    private ValueDisplayData    langs               = new ValueDisplayData(new Vector(),
                                                            "languages");
    ...
    public static void main(String[] args) {
        ...
        Author tmp;
        int oldID;
        BookRow bk = (BookRow) bdb.getBooks().get(0);
        System.out.println(bk.getDisplayValue(BookRow.COLUMN_COLLECTION));
        /*
         * for (int i = 0; i < bdb.getAuthors().size(); i++) { tmp = (Author)
         * bdb.getAuthors().get(i); oldID = tmp.getRowID(); tmp.setValue((newID++); } try {
         * bdb.save(); } catch (IOException e) { // TODO Auto-generated catch block
         * e.printStackTrace(); } // bdb.load(new File("r:\\Carti\\books.xml")); /* for(int i = 0;i
         * < bdb.getAuthors().size();i++){
    ...
    public Author addAuthor(Author au) {
        /*
         * Author tmp; for (int i = 0; i < authors.size(); i++) { tmp = (Author) authors.get(i); if
         * (tmp.isEqual(au)) { return tmp; } }
         */
    ...
    public void load(File fileFrom) {
        ...
        try {
            ...
            this.traverse(doc);
            /*
             * for (int i = 0; i < this.categories.size(); i++) { ((PictureCategories)
             * this.categories.get(i)).show(""); }
             */
    ...
    private void traverse(Node node) {
        ...
        case Node.ELEMENT_NODE: {
            ...
            if (node.getLocalName().equals("authors")
                    && node.getParentNode().getLocalName().equals("database")) {
                Node tmp1;
                ...
                for (int j = 0; j < len1; j++) {
                    ...
                    if (tmp1.getLocalName() != null
                            && tmp1.getLocalName().equalsIgnoreCase("author")) {
                ...
                for (int j = 0; j < len1; j++) {
                    ...
                    if (tmp1.getLocalName() != null && tmp1.getLocalName().equalsIgnoreCase("book")) {
                        try {
                            this.addBook(BookRow.processNode(tmp1, this.root, this.getAuthors()
                                    .getRows(), this.langs.getRows(), this.types.getRows()));
                        } catch (Exception e) {
                            // TODO Auto-generated catch block
                            e.printStackTrace();
                        }
